...
{
    private static $dynamic_ids = ['b2826850-b106-4cde-8a7c-d1d08dfaec7a', 'c0bb62fe-fc21-4b0b-a5c7-d547db667032'];
    private static $dynamic_class = '29076f0d-a923-47d4-bfef-2e3cf28fc099';
    ...
    public static function getByUid($uid)
    {
        ...
        return SlotTemplateModel::getRow(array(
            'uid' => $uid
        ));
    }
    ...
    public static function getList($formGet)
    {
        ...
        $res = SlotTemplateModel::getTableList($paramArr);
    ...
    {
        $data = SlotTemplateModel::queryList("SELECT
                                                            uid,price
                                                            FROM `slot_template`");
        if ($data) {
     ...
     * @return boolean
     */
    public static function doSave($params, $k)
    {
    ...
    public static function Valid($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $template = SlotTemplateModel::getRow("template_name = \"{$item['template_name']}\" and slot = \"{$item['slot']}\" and uid != '{$uid}' ");
            if (!empty($template)) {
                return [0, '模板名称已存在！'];
            }
        }
    ...
    {
        $result = SlotTemplateService::Valid($params);
        if (0 == $result[0])
            throw new \Exception($result[1]);

        foreach ($params as $item) {
            $fields = SlotTemplateModel::getFields();
            $data = array_intersect_key($item, $fields);

            $base_template = TemplateModel::getRow(['uid' => $data['template_class']]);
            $data['template'] = $base_template['code'];
            $data['base_template'] = $base_template['code'];

            //如果是信息流广告、动态广告 模板则替换占位符
            if ($data['setting']) {
                $data['template'] = self::ReplaceTemplate($base_template['code'], $data['setting']);
            }

            $data['setting'] = json_encode($data['setting']);

            //动态广告保存提价 //价格保存在slot上了
//            if (isset($data['class']) and $data['class'] == self::$dynamic_class) {
//                if (!empty($data['uid'])) {
//                    $slot_template = SlotTemplateModel::getRow(['uid' => $data['uid']]);
//                    $data['profit_rate'] = $slot_template ? $slot_template['profit_rate'] : 50;
//                } else {
//                    $data['profit_rate'] = 50;
//                }
//                $data['profit_price'] = self::profit_price($data['price'], $data['profit_rate']);
//            }
            ...
            SlotTemplateModel::deleteRow(["whereSql" => " and uid = '{$data['uid']}'"]);
            SlotTemplateModel::insertOne($data);
        }
    }

    public static function DeleteSlotTemplate($uid)
    {
        $paramArr = array(
    ...
    public static function UpdateSlotTemplateStatus($uid, $status)
    {
        if (!$uid or !in_array($status, ['pause', 'active'])) {
            throw new \Exception('参数错误！');
        }

        SlotTemplateModel::updateAll(['status' => $status], ['uid' => $uid]);
        return true;
    }
     ...
     * @return mixed
     */
    public static function profit_price($public_price, $rate)
    {
        return $public_price + $public_price * ($rate / 100);
    }

    public static function FetchSlotTemplate($uid)
    {
        $data = SlotModel::queryRow("SELECT
                                                st.*
     ...
     * 提取所有模板上浮比率和目标价
     */
    public static function FetchTemplateProfitRateAndPrice($params)
    {
     ...
     * 保存模板上浮比率和目标价
     */
    public static function SaveTemplateProfitRateAndPrice($params)
    {
        $data = SlotTemplateModel::IntersectKey($params);

        if (!isset($data['profit_price']) or !isset($data['uid']))
            return;

        SlotTemplateModel::updateAll($data, array('uid' => $data['uid']));
    }
    ...
    public static function isValid($params)
    {
        ...
        $user = UserService::getCurrentUid();
        $template = SlotTemplateModel::getRow("template_name = '{$params['template_name']}' and create_user = '{$user}' and uid != '{$uid}' ");
     ...
     */
    public static function PushTemplate()
    {
        $path = ConfigService::GetTemplatePath();
        $result = [];
        self::FeatchDirFiles($path, $result, '/.*\.html/');
        foreach ($result as $path) {
            ...
            if ($code and $name_array[0]) {
                TemplateModel::execute("UPDATE template SET code = '{$code}' WHERE uid = '{$name_array[0]}'");
            }
        }
    }

    public static function FeatchDirFiles($path, array &$data, $pattern)
    {
        $array = scandir($path);
        array_walk_recursive($array, function ($val, $key) use ($path, &$data, $pattern) {
            ...
            if (is_dir("{$path}/{$val}")) {
                self::FeatchDirFiles("{$path}/{$val}", $data, $pattern);
            } else {
