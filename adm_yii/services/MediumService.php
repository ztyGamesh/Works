...
class MediumService
{
    ...
    public static function ReplaceMeidumInfo(array $data)
    {
        if(!$data)
     ...
     * @param $audit_type
     */
    public static function getMediumByAuditType($audit_type)
    {
        if(!$audit_type or !in_array($audit_type,['self', 'self_other']))
            throw new \Exception('parameter wrong');

        return MediumModel::getList(['audit_type'=>$audit_type]);
    }

    /**
     * 返回全量媒体账户
     */
    public static function getAllMedium()
    {
        return MediumModel::queryList("
        SELECT 
        u.`uid`,
     ...
     * @param $uid
     */
    public static function isAdx($uid){
        $row= MediumModel::queryRow("select media_type from medium where media_type='adx' and uid='{$uid}'");
        return !empty($row);
    }
