...
class DrawLogService
{
    ...
    public static function getmediadrawlist($params=[])
    {
        $condition = new ConditionCombineModel();
    ...
    public static function drawdetail($params)
    {
        if (!isset($params['id'])) {
    ...
    public static function paytomedia($params)
    {
    ...
    public static function checkdraw()
    {
        //申请时间范围检查
        ...
        $draw_check = DrawLogModel::queryRow("
        SELECT 1
        FROM `draw_log`
        WHERE 
        ...
        $data = self::getaccountfinanceinfo();
     ...
     * @throws \Exception
     */
    public static function applydraw($params)
    {
        ...
        $check_finance = MediumModel::getRow(['uid' => $uid]);
        if (!$check_finance)
            throw new \Exception('请补齐媒体银行账户信息！');

        //申请余额检查
        $remain_check = DrawLogModel::queryRow("
        SELECT 
        ((SELECT 
     ...
     */
    public static function getaccountfinanceinfo()
    {
        $uid = UserService::getCurrentUid();

        return DrawLogModel::queryRow("
        SELECT 
     ...
     * 计算时间范围内媒体账户累计收入
     */
    public static function ComputeAllianceMediaTotalIncome($params)
    {
...
//        $params['end'] = $params['end'] ? $params['end'] : date('Y-m-d', strtotime('-1 day'));
     ...
     * 计算联盟媒体时间范围内月账单
     */
    public static function ComputeAllianceMediaBill($params)
    {
     ...
     * @return bool
     */
    public static function ComputeAllianceLastMonthMediaBill()
    {
        $params['begin'] = date('Y-m-01', strtotime('-1 month'));
        $params['end'] = date('Y-m-t', strtotime('-1 month'));

        self::ComputeAllianceMediaBill($params);
        return true;
    }
     ...
     */
    public static function ComputeAllianceWholeMediaBill()
    {
        $params['begin'] = '1970-01-01';
        $params['end'] = date('Y-m-t', strtotime('-1 month'));
