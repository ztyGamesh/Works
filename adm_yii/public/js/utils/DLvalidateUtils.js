...
// 金额校验(50~9999999999.99)
function vaIsMoneyFrom50NotNull(value) {
    var str = (value + '').trim();
    var reg = /(^[1-9]\d{0,9}$)|(^[1-9]\d{0,9}\.\d{1,2}$)/;
    return  +str >= 50 && reg.test(str);
}
...
// 校验输入字符数为1-40个字符
function vaWordNumberLimit1_20(str) {
    var trimStr = str.replace(/(^\s*)|(\s*$)/g, '');//去掉空格
    var wordLength = trimStr.replace(/[^x00-xFF]/g, '**').length;
    return (wordLength >= 1 && wordLength <= 40);
}
...
//匹配1~50之间的正整数
function isPositiveInt1_50(str) {
    var reg = /^(:?[1-9]|[1-4]\d|50)$/;
    return reg.test(str);
}
//验证输入是否是字母和数字
