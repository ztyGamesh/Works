...
//多选控件
 function addAreaRep(id, value,_id) {
 	var ori = id;
 	id = "tr_" + id;
 	if ($("#"+_id+"-areaSelected #" + id).length <= 0)
 		$("#"+_id+"-areaSelected").append("<dt id='" + id + "'><a class='fr' href='javascript:void(0)' onclick=unCheckArea('" + ori + "','" +_id+"')>删除</a><strong>" + value + "</strong></dt>");
 }
 ...
 function regionList(id) {
 	...
 	btnB2.appendChild(areaSearchButton);
 	...
 	for (var key = 0; key < selectList.options.length; key++) {
 		...
 		if (!_option.hasAttribute('parent-id')) {
 			...
 			if (_option.hasAttribute('checked') || _option.hasAttribute('selected')) {
				...
				addAreaRep(id+'-area'+key,_option.text);
                }
                ...
                if(!tempFlag){
                    ...
                    if(_option.hasAttribute('checked') || _option.hasAttribute('selected')){
                        ...
                        addAreaRep(id+'-area'+key,tempText+' · '+_option.text);
                    }
        ...
        $("#"+id+" div.areaL input[id^='"+id+"-area']:checked").each(function() {
            addAreaRep($(this).attr('id'), $(this).attr('data-text') , id);
        });
