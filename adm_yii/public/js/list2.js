...
var List = {
    ...
    htmlConfig: {
        ...
        //     '</button>'],
        // info: ['<button class="info btn btn-xs btn-info btn-group-xs btn-default"  title="查看">',
        //     '<i class="glyphicon glyphicon-th"></i>',
        //     '</button>'],
...
//     },
    ...
    editItem: function (indexId) {
        //订单结束后，不能修改订单及广告投放的设置
        //开始
        var state = true;
        if (ListPageConfig.validate.vaPause !== undefined) {
            getorderinfoAPI(indexId, getorderinfoAPISuccess);
            function getorderinfoAPISuccess(result) {
                var res = JSON.parse(result);
                if (res.status == '1') {
                    if (res.data.pause == 4) {
                        layer.msg("状态为'结束'的订单不可编辑");
                        state = false;
                    }
                } else {
                    console.log("获取订单信息错误：" + res.msg);
                }
            }
        }
        if(!state){
