...
export default class CustomRangePicker extends Component {
    ...
    dateChange (dates, dateString) {
        ...
        if (this.props.haveAction) {
            this.props.haveAction(dates, dateString)
        }
    	this.setState({
    		value: dates
    	})
    ...
    checkboxChange (e) {
        ...
        this.props.isCompare(e.target.checked)
        this.setState({
    ...
    render() {
        return (
            <div>
                {this.props.checkbox ? <Checkbox onChange={this.checkboxChange.bind(this)}></Checkbox> : null}
                <RangePicker
                    disabled={this.props.checkbox ? this.state.disabled : false}
                    style={{width:"80%"}}
                    ...
                    ranges={
                        {
                            "今天":[todaySubtract(0),todaySubtract(0)],
