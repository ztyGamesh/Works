...
function comebine(current, compare) {
	...
	if (current.length  === 1 ) {
		...
		flag = current[0].date === current[1].date ? "day" : "period";
	}
	...
	if (flag === "day") {
		...
		return current.map((currentItem, index) => {
			currentItem.xValue = currentItem.hour;
		...
		// 如果是某一天的数据curren&compare
		return current.map((currentItem, index) => {
			currentItem.xValue = currentItem.period_date;
			currentItem.income = parseFloat(currentItem.income);
			currentItem.imp = parseInt(currentItem.imp);
			currentItem.ctr = parseFloat(currentItem.ctr);
			currentItem.clk = parseInt(currentItem.clk)
...
class LineChart extends Component {
	...
	componentWillMount() {
		...
		console.log("showCompare", this.props.showCompare)
		...
		// const dataSource = comebine([{date:"1"},{date:"1"}],[{},{}])
		// console.log(dataSource)
		const dv = new View().source(dataSource);
		let compareFlag = this.props.showCompare;
		if (compareFlag) {
			dv.transform({
				type: 'rename',
				map: {
		...
		this.setState({
			...
			showCompare: compareFlag
		})

	}
	componentWillReceiveProps(nextProps) {
		// console.log("数据源",nextProps)
		console.log("showCompare", nextProps.showCompare)
		console.log("do")
		if (nextProps.lineChartCurrent.length === nextProps.lineChartCompare.length) {
			...
			let compareFlag = nextProps.showCompare;
			if (compareFlag) {
				dv.transform({
					type: 'rename',
					map: {
	...
	draw(activeButtons) {
		console.log("draw++++++", this.state.showCompare)
		let fieldsArr;
		let compareFlag = this.state.showCompare;
		if (compareFlag) {
			fieldsArr = activeButtons.map((x) => {
				return [x, x + "Compare"]
			})
		} else {
			fieldsArr = activeButtons.map((x) => {
		...
		fieldsArr.forEach((fields) => {
			let renameFields = compareFlag ? this.renameCompare(fields) : this.rename(fields);
...
const mapStateToProps = (state) => {
	return {
		lineChartCurrent: state.creativeReportReducers.current.chart[0],
		lineChartCompare: state.creativeReportReducers.compare.chart[0]
	}
}
