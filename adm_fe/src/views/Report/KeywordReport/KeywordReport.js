...
import Filter from '../../../containers/Report/KeywordReport/Filter';
import LineChart from '../../../containers/Report/KeywordReport/LineChart';
import TableForReport from '../../../containers/Report/KeywordReport/TableForReport';
import TableDimension from '../../../containers/Report/KeywordReport/TableDimension';
import AdsDropDown from '../../../containers/Report/KeywordReport/AdsDropDown';
import Toggle from '../../../containers/Report/KeywordReport/Toggle';
...
import {allianceclient,currentSetData,compareSetData, deltaTimeSet,fetchuseradstruct,useradstructSet} from './flow/KeywordReportActions.js';
import {wordColumnsunlimited,wordColumnsdate,wordColumnsword} from '../../../mock/Report/ColumnsMap';
...
class GroupReport extends Component {
	constructor(props) {
		...
		this.state = {
			...
			delta: 0,
			// 从mock处取得账户报表页面的报表列
			columns: Object.assign([], wordColumnsword),
            // 帮助表格组件存放查询条件的变量
			forReport: {
				dateA:{
					begin: todaySubtract(0),
	...
	updateHandle = async (params) => {
		...
		const res = await request({
			url: `${BUSINESSAPIHOST}/alliancereport/wordreport`,
			method: 'get',
			data: {
	...
	renderColumns(columnsNames) {
		...
		switch (this.state.reportType) {
			case "unlimited":
				columnsModel = wordColumnsunlimited;
				break;
			case "date":
				columnsModel = wordColumnsdate;
				break;
			case "word":
				columnsModel = wordColumnsword;
				break;
			default:
				return
		}
	...
	handleUpload() {
		// 下载表单
		var x = BUSINESSAPIHOST + '/alliancereport/exportcsv?action_name=wordreportAction&offset=0&limit=1000000&condition=' + this.state.sorter.condition;
	...
	toggle(value) {
		...
		switch (value) {
			case "unlimited":
				currentColumes = wordColumnsunlimited;
				sorter.field = "imp";
				sorter.order = "descend";
				break;
			case "date":
				currentColumes = wordColumnsdate;
				sorter.field = "period_date";
				sorter.order = "descend";
				break;
			case "word":
				currentColumes = wordColumnsword;
				sorter.field = "imp";
				sorter.order = "descend";
				break;
			default:
