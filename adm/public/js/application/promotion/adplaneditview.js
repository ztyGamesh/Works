    ...
    function fetchadplanAPIonSuccessFun(result) {
        ...
        if (req.status == 1) {
            ...
            if (data.platform !== '') {
                switch (purpose) {
                    case 'download':
                        $("#platCheck option[value = " + data.platform + "]").prop('selected', 'selected');
                        $("#platCheck option[value = " + data.platform + "]").attr('selected', 'selected');
                        break;
                    case 'landing':
            ...
            if (data.media_target !== '') {
                lastmedia = data.media_target; //先存储已经选中的，再去跟全部的对比
            }
            //广告行为定向选中部分
            if (data.tag_target !== '') {
                lastAdTarget = data.tag_target;
                $("#advertising-behavior-orientation option[value = 1]").prop('selected', 'selected');
                $("#ad-behavior-group").show();
                $("#dl-list").show();
                isShow = true;
            }
            if (data.network !== '') {
            ...
            initdaterangepickerPlan($("#searchDate"), moment(data.s), moment(data.e), moment(data.s));
            ...
            if(data.class == 'c96089f7-9cff-4149-997f-bb03d617cda0'){//固定信息流
                $('#bid_type option[value=cpm]').prop('selected', 'selected').change();
            ...
            if (req.status == 1) {
                ...
                if (req.data.length == 0) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        var isSelected = false; //用于标记当前数据是否是选中状态，true:选中状态
                        for (var j = 0; j < lastmedia.length; j++) {
                            if (lastmedia[j].bundle_id == data[i].bundle_id) {
                                isSelected = true;
                            }
                        }
                        if (isSelected) {
                            html += "<option value='" + data[i].bundle_id + "'selected>" + data[i].name + "</option>"
                        }
                        else {
...
// 路径导航栏 开始
    ...
    //广告行为定向
    $("#advertising-behavior-orientation").bind('change', function () {
    ...
    function tagAPIonSuccess(result) {
        ...
        if (req.status == 1) {
            ...
            for (var i = 0; i < lastAdTarget.length; i++) {
                editItializeiFun(lastAdTarget[i].level2);
            }
        }
        else
        {
    ...
    $(".dl-list-footer-cancel-btn").bind('click', function () {
        refreshMultiMenu();//刷新
        for (var i = 0; i < lastAdTarget.length; i++) {
            editItializeiFun(lastAdTarget[i].level2);//重新勾选
        }
        $("#advertising-behavior-orientation-title").html('自定义广告行为定向可多选');
        $("#dl-list").hide();
        isShow = false;
    ...
    $("#submit").on('click', function () {
        ...
        if(mediaSelected_length == list_lenght)
        {
            mediaSelected = '';
        ...
        setTimeout(function () {
            adplansaveeditAPI(id, planName, group_id, slot_class, area, platform, network, mediaSelected, key_word, channel_class, s, e, budget, bid_type, price, frequency, speed, tagTarget,function (result) {
                $("#Deepleaper_loading").hide();
                var req = JSON.parse(result);
                if (req.status == 1) {
                    layer.msg('广告计划修改成功');
                    setTimeout(function () {
                        window.location = '/promotion/adplanlistview?groupId=' + GetQueryString('groupId');
                    }, 1000);
                } else {
                    layer.msg(req.msg);
                }
