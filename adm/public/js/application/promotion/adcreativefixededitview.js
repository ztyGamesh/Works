/**
 * Created by liuchangyu on 17/06/13.
 */
...
$(document).ready(function () {
    //创建创意获取支持的样式数据
    fetchplantemplatesAPI(GetQueryString('planId'), function (result) {
        var req = JSON.parse(result);
        if (req.status == 1) {
            var data = req.data;
            for (var i = 0; i < data.length; i++) {
                var id = data[i].template_class;
                var setting = JSON.parse(data[i].setting);
                $("#" + id + " .creativeTitle").attr({
                    'maxlength': setting.title_setting.length ,
                    'placeholder': '6~' + setting.title_setting.length + '个字'
                });
                var picSizeHtml = '';
                var pic_size = setting.pic_setting.pic_size;
                for (var j = 0; j < pic_size.length; j++) {
                    picSizeHtml += '<span class="colorpicmsg">' + pic_size[j] + '</span><br/>';
                }
                $("#" + id + " .pic_size").html(picSizeHtml);
                $("#ad_source").attr({
                    'maxlength': setting.ad_source_setting.length,
                    'placeholder': '2~' + setting.ad_source_setting.length + '个字'
                })
            }
        } else {
            layer.msg(req.msg);
    ...
    function fetchcreativeAPIonSuccessFun(result) {
        ...
        if (req.status == 1) {
            ...
            if (material.pic) {
                $("#" + id + " .inputUpload_image").attr('initflag', material.pic);
                $("#" + id + " .demonstrate_image").attr('src', 'http://static.adm.deepleaper.com/material/' + material.pic);
            }
            if (material.pic_scale) {
                $("#" + id + " .inputUpload_image").attr('initflag', material.pic);
                $("#" + id + " .inputUpload_image").attr('pic_scale', material.pic_scale);
                $("#" + id + " .demonstrate_image").attr('src', 'http://static.adm.deepleaper.com/material/' + material.pic);
            }
            if (material.video) {
                $("#" + id + " .inputUpload_video").attr('initflag', material.video);
            ...
            if (id == '2c52b03e-b044-4fda-8c18-bc00ed3bd80d') {
                $("#" + id + " .inputUpload_image")[0].setAttribute('initflag', material.pic1);
                $("#" + id + " .inputUpload_image")[1].setAttribute('initflag', material.pic2);
                $("#" + id + " .inputUpload_image")[2].setAttribute('initflag', material.pic3);
...
// 输入校验
    ...
    var downloadCheckList = [
        {
            id: '#appName',
            validationFunc: [vaIsNotNull, va2_8],
            msg: ['应用名不能为空', '应用名2-8个字']
        }, {
            id: '#appDownloadUrl',
            ...
            id: '#appPackageName',
            validationFunc: [vaAppPackageName],
            msg: ['应用包名称不能为空']
        }
    ];
    ...
    function vaAppPackageName(str) {//应用包名称在android情况下必填
        if (platform == 'android') {
            return vaIsNotNull(str);
        } else {
            return true;
        }
    }

    function va2_8(str) {//广告来源2-8个字
        var wordLength = str.length;
        return (wordLength >= 2 && wordLength <= 8);
    }

    function va2_12(str) {//创意副标题2-12个字
        var wordLength = str.length;
        return (wordLength >= 2 && wordLength <= 12);
    }
    ...
    $('#landing_extend_title').bind('blur', function () {
        if (!vaIsNotNull($(this).val()) || !va2_12($(this).val())) {
    ...
    $("#submit").bind('click', function () {
        ...
        var creative_id = GetQueryString('id');
        var link = $("#link").val();
        ...
        var bundle_id = $("#appPackageName").val();
        if (purpose == 'landing') {
            if (!landingpresubmit()) {
                return false;
            }
        ...
        templatectl.each(function (index, element) {
            ...
            inputfile.each(function (index, element) {//没有成功上传图片的，使用初始化加载图片
                if ($(this).attr('flag') == 'false') {
                    $(this).attr('flag', $(this).attr('initflag'))
                }
            });

            // 测试创意名称字数限制
            var creativeName = collapse.find('.creativeName');
            if (!vaWordNumberLimit1_25(creativeName.val())) {
                creativeName.focus();
                inputfileisSucc = false;
                vaError(creativeName, '名称长度应为1-50个字符:汉字占2个字符');
                return false;
            } else {
                vaSuccess(creativeName);
            }

            var titleTextarea = collapse.find('.creativeTitle');
            if (titleTextarea.length !== 0) {
                var str = titleTextarea.val().length;
                if (str < 6 || str > 17) {
                    inputfileisSucc = false;
                    titleTextarea.focus();
            ...
            if (id == '2c52b03e-b044-4fda-8c18-bc00ed3bd80d') {//组图模板有3张图
        ...
        adcreativeeditAPI(adcreativesaveObj, function (result) {
            var req = JSON.parse(result);
            if (req.status == 1) {
                $("#Deepleaper_loading").hide();
                layer.msg('创意修改成功');
                setTimeout(function () {
                    window.location = '/promotion/adcreativelistview?planId=' + GetQueryString('planId') + '&groupId=' + GetQueryString('groupId');
                }, 1000);
            } else {
                $("#Deepleaper_loading").hide();
                layer.msg(req.msg);
        ...
        // layer.confirm('即将在不改变现有创意的基础上添加一条新的创意，是否继续?', function (index) {
        //     layer.close(index);
        //     adcreativeeditAPI(adcreativesaveObj, function (result) {
        //         var req = JSON.parse(result);
        //         if (req.status == 1) {
        //             layer.alert('创意修改成功，已新建一条新的创意', function (index) {
        //                 window.location = '/promotion/adcreativelistview?planId=' + GetQueryString('planId') + '&groupId=' + GetQueryString('groupId');
        //             });
        //         } else {
        //             layer.msg(req.msg);
                            ...
                            imgLeft:{
                                set:function(newValue) {
                                    if (!newValue) {
                                        ...
                                        $("#"+fatherID+" .main_img img")[0].src = newValue;
                                    } else {
                                        $("#"+fatherID+" .main_img img")[0].src = newValue.src;
                                    }
                                }
                            },
                            imgCenter:{
                                set:function(newValue) {
                                    if (!newValue) {
                                        newValue = "../../../images/placeHolder.png";
                                        $("#"+fatherID+" .main_img img")[1].src = newValue;
                                    } else {
                                        $("#"+fatherID+" .main_img img")[1].src = newValue.src;
                                    }
                                }
                            },
                            imgRight:{
                                set:function(newValue) {
                                    if (!newValue) {
                                        newValue = "../../../images/placeHolder.png";
                                        $("#"+fatherID+" .main_img img")[2].src = newValue;
                                    } else {
                                        $("#"+fatherID+" .main_img img")[2].src = newValue.src;
                                    }
                                }
                            }
                        });
    ...
    Object.defineProperty(obj,"advButton",{
        set:function(newVal) {
            for(var i = 0,len = $(".title_button").length;i < len;i ++) {
                ...
                if (flag == "无" ) {
                    if (purpose == "landing" || purpose == "download") {
                        $(".main_title").hide();
                    }
                } else {
                    // $(".main_title").show();
    ...
    /*预览模板初始化*/
    {
        var preModuelInit = {
            ...
            titleHandler:function() {
                var preModuelInit = this.titleModelId;
                for (var preId in preModuelInit) {
                    var $father = $("#" + preId);
                    var $previewFather = $("#preview"+preId);

                    var $title = $father.find(".creativeTitle");
                    var $title_pre = $previewFather.find(".adv_title");

                    if ($title_pre[0]) {
                        $title_pre[0].innerHTML = $title.val();
                    }

                }
            },
            imgHandler:function() {
                var preModuelInit = this.imgModelId;
                for (var preId in preModuelInit) {
                    var $father = $("#" + preId);
                    var $previewFather = $("#preview"+preId);


                    var $img_pre = $previewFather.find(".main_img img");
                    var $img = $father.find(".demonstrate_image");


                    //组图特殊处理
                    if (preId == "collapse_infor_flow_imgs") {
                        $img_pre[0].src = $img[0].src;
                        $img_pre[1].src = $img[1].src;
                        $img_pre[2].src = $img[2].src;
                    }
                    if ($img_pre[0]) {
                        $img_pre[0].src = $img[0].src;
                    }
            ...
            commonHandler:function() {
                var preModuelInit = this.commonModelId;
                for (var preId in preModuelInit) {
                    var $father = $("#" + preId);
                    var $previewFather = $("#preview"+preId);

                    var $adSource ;
                    var $buttonDertermine = $("#extend_type option:selected").html();
                    var $buttonName;
                    var $addSource;
                    if (purpose == "landing") {
                        $addSource = $("#landing_extend_title");
                        $adSource = $("#ad_source");
                    } else if (purpose == "download") {
                        $addSource = $("#download_extend_title");
                        $adSource = $("#appName");
                    }


                    var $adSource_pre = $previewFather.find(".adv_addsource");
                    var $buttonName_pre = $previewFather.find(".title_button");
                    var $addSource_pre = $previewFather.find(".adv_addtitle");


                    //根据$buttonDetermine判断
                    for (var ii = 0;ii < $buttonDertermine.length;ii++) {
                        if ($buttonDertermine[ii].selected) {
                            $buttonDertermine = $buttonDertermine[ii].innerHTML;
                            break;
                        }
                    }
                    //给$button赋值

                    switch ($buttonDertermine) {
                        case "无" :
                            $buttonName = "按钮名称";
                            $(".main_title").hide();
                            break;
                        case "电话拨打":
                            $buttonName = $("#phone_btnName option:selected").html();
                            break;
                        case "表单收集":
                            $buttonName = $("#form_btnName").val();
                            break;
                        default:
                            console.log("按钮部分代码执行了");
                    }

                    if ($adSource_pre[0]) {
                        $adSource_pre[0].innerHTML = $adSource.val();
                        if (purpose == "landing") {
                            $buttonName_pre[0].innerHTML = $buttonName;
                        } else if (purpose == "download") {
                            $buttonName_pre[0].innerHTML = "应用下载";
                        }

                        $addSource_pre[0].innerHTML = $addSource.val();
                    }
