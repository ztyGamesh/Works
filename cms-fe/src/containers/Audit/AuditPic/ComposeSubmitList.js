...
class SubmitList extends Component {
    ...
    saveCompose(auditStatus, refuseComment, refuseConfirm) {
        ...
        const {title, marksman, content, editorContent, displayInfo, category, tags, channel, score, machineTag, type, uid, newContent} = this.props;
    ...
    handleShopData() {
        ...
        console.log(content);
        for (var i = 0; i < content.length; i++) {
            if (content[i].pic && content[i].goodsPromotion.length !== 0 && content[i].goodsPromotion[0].uid !== "") {
                console.log("看看图集的中的商品格式");
                console.log(content[i].goodsPromotion);
                shopListData.push({
